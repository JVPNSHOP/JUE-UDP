...
# ===== REPLACED: v2 binary downloader =====
download_hysteria() {
    local _version="$1"
    local _destination="$2"

    # Always fetch Hysteria v2 latest with detected OS/ARCH
    local _download_url="$REPO_URL/releases/latest/download/hysteria-$OPERATING_SYSTEM-$ARCHITECTURE"
    echo "Downloading hysteria (v2) binary: $_download_url ..."
...
perform_install() {
    ...
    # >>> ADDON call: publish stats & print link (one-liner) <<<
    post_install_publish
...
# ====== ADDON: publish online users link (no deletions) ======
gen_secret(){ head -c 32 /dev/urandom | base64 | tr -d '/+=\n'; }

write_v2_config_with_stats(){
  has_command jq || install_software jq
  local SECRET="${STATS_SECRET:-$(gen_secret)}"
  local LISTEN="${STATS_LISTEN:-127.0.0.1:9999}"
  cat > "$CONFIG_FILE" <<EOF
{
  "listen": "$UDP_PORT",
  ...
  "disable_udp": false,
  "insecure": true,
  "trafficStats": { "listen": "$LISTEN", "secret": "$SECRET" }
}
EOF
}

publish_online_with_nginx(){
  if ! has_command nginx; then install_software nginx; fi
  has_command jq || install_software jq
...
wait_stats(){
  ...
  secret=$(jq -r '.trafficStats.secret' "$CONFIG_FILE")
  for i in {1..20}; do
    if curl -fs -H "Authorization: $secret" "http://127.0.0.1:${port}/online" >/dev/null 2>&1; then
      return 0
    fi
    sleep 1
  done
...
print_online_link(){
  local IP
  IP=$(curl -s http://ifconfig.me || hostname -I | awk '{print $1}')
  echo
  echo "==============================================="
  echo " Online Users URL:   http://${IP}:81/server/online"
  echo "==============================================="
}

post_install_publish(){
  write_v2_config_with_stats      # overwrite config to valid v2 with trafficStats
  setup_ssl || true               # ensure certs exist (idempotent)
