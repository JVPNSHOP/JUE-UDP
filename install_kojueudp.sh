...
get_actual_online_users() {
    ...
    if has_command ss; then
        ...
        online_count=$(ss -uap 2>/dev/null | grep ":$port" | grep -v "LISTEN" | wc -l 2>/dev/null || echo "0")
    fi
    echo "$online_count"
}
...
setup_web_dashboard() {
    ...
    # Create main dashboard page with dynamic IP
...
EOF
    ...
    # Create JSON API endpoint with dynamic IP
...
function getOnlineUsers() {
    ...
    // Method 1: Using ss command to count UDP connections to Hysteria port
    if (function_exists('shell_exec')) {
        $output = shell_exec("ss -uap 2>/dev/null | grep ':36712' | grep -v 'LISTEN' | wc -l");
        $online_users = intval(trim($output));
    }
    
    // Method 2: Alternative using netstat
    if ($online_users == 0 && function_exists('shell_exec')) {
        $output = shell_exec("netstat -nu 2>/dev/null | grep ':36712' | wc -l");
        $online_users = intval(trim($output));
    }
...
function getServerUptime() {
    if (function_exists('shell_exec')) {
        ...
        if (trim($output) === 'active') {
            $output = shell_exec("systemctl show hysteria-server.service --property=ActiveEnterTimestamp --value 2>/dev/null");
...
EOF
    ...
    # Create Text API endpoint with dynamic IP
...
echo "Online Users: " . $online_users . PHP_EOL;
