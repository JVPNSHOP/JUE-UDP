...
count_hysteria_users() {
    ...
    if command -v ss >/dev/null 2>&1; then
        count=$(ss -u -n state established 2>/dev/null | grep ":$port" | wc -l)
    fi
    
    # Method 2: Use netstat as fallback
    if [ "$count" -eq 0 ] && command -v netstat >/dev/null 2>&1; then
        count=$(netstat -nu 2>/dev/null | grep ":$port" | grep -v "127.0.0.1" | wc -l)
    fi
    
    # Method 3: Check Hysteria server process connections
    if pidof hysteria >/dev/null; then
        local hysteria_pid=$(pidof hysteria)
        if [ -n "$hysteria_pid" ]; then
            local process_count=$(lsof -i udp:$port -a -p $hysteria_pid 2>/dev/null | grep -v "LISTEN" | wc -l)
    ...
    # Method 4: Check /proc/net/udp for connections
    if [ "$count" -eq 0 ]; then
        local hex_port=$(printf "%04X" $port)
        count=$(grep ":$hex_port" /proc/net/udp 2>/dev/null | grep -v "00000000:0000" | wc -l)
    fi
...
check_hysteria_status() {
    ...
    if pgrep -f "hysteria server" > /dev/null 2>&1; then
        ...
        if systemctl is-active hysteria-server.service > /dev/null 2>&1; then
            ...
            if ss -uln | grep ":36712" > /dev/null 2>&1; then
                status="running"
            else
                status="stopped"
            fi
            ...
            <div class="server-card">
                ...
                <div class="server-details">
                    <div class="detail-item">
                        ...
                        <span class="detail-value" id="server-ip"><?php echo $server_ip; ?></span>
                    </div>
                    <div class="detail-item">
...
EOF
    ...
    # Create JSON API endpoint with IMPROVED service status checking
...
function getOnlineUsers() {
    ...
    if (function_exists('shell_exec')) {
        $output = shell_exec("/usr/local/bin/count_hysteria_users.sh 2>/dev/null");
    ...
    if ($online_users == 0 && function_exists('shell_exec')) {
        $output = shell_exec("ss -u -n state established 2>/dev/null | grep ':36712' | wc -l");
        $online_users = intval(trim($output));
    }
    
    // Method 3: Check active UDP connections using netstat
    if ($online_users == 0 && function_exists('shell_exec')) {
        $output = shell_exec("netstat -nu 2>/dev/null | grep ':36712' | grep -v '127.0.0.1' | wc -l");
        $online_users = intval(trim($output));
    }
...
function getServerStatus() {
    ...
    if (function_exists('shell_exec')) {
        $output = shell_exec("/usr/local/bin/check_hysteria_status.sh 2>/dev/null");
        $status = trim($output);
        
        if (!empty($status) && ($status === 'running' || $status === 'stopped')) {
            return $status;
        }
    }
    ...
    // Fallback method: Check systemd service
    if (function_exists('shell_exec')) {
        $service_check = shell_exec("systemctl is-active hysteria-server.service 2>/dev/null");
        if (trim($service_check) === 'active') {
            return 'running';
        }
    }
    
    // Fallback method: Check if port is listening
    if (function_exists('shell_exec')) {
        $port_check = shell_exec("ss -uln | grep ':36712' 2>/dev/null | wc -l");
        if (intval(trim($port_check)) > 0) {
            return 'running';
        }
    }
    
    // Final fallback: Check if hysteria binary is executable
    if (is_executable("/usr/local/bin/hysteria")) {
        $version_check = shell_exec("/usr/local/bin/hysteria version 2>/dev/null");
...
function getServerUptime() {
    if (function_exists('shell_exec')) {
        ...
        // Try systemd service info
        $service_info = shell_exec("systemctl show hysteria-server.service --property=ActiveState --value 2>/dev/null");
        if (trim($service_info) === 'active') {
            $service_start = shell_exec("systemctl show hysteria-server.service --property=ActiveEnterTimestamp --value 2>/dev/null");
...
EOF
    ...
    # Create Text API endpoint with RELIABLE counting
...
echo "Counting Method: Multiple UDP connection detection" . PHP_EOL;
...
EOF
    ...
    TEST_COUNT=$(/usr/local/bin/count_hysteria_users.sh)
    echo "Current connected users: $TEST_COUNT"
    
    echo "Testing service status checking..."
    TEST_STATUS=$(/usr/local/bin/check_hysteria_status.sh)
    ...
    echo "   ✅ Multiple UDP connection detection methods"
