...
check_hysteria_direct() {
    if ps aux | grep -v grep | grep "hysteria server" > /dev/null; then
        echo "running"
    else
        echo "stopped"
    fi
...
main_status=$(check_hysteria_status)

# If still unknown, try direct check
if [ "$main_status" = "unknown" ]; then
    main_status=$(check_hysteria_direct)
fi
...
EOF
    ...
    # Create JSON API endpoint with RELIABLE service status checking
